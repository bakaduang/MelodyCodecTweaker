...
## 2.5.0-preview.8：Android 17 / 无线耳机 17.5.1 适配

- 根据 PMA120 / ColorOS 17.0.0.103 的反馈库，新增带字符串引用参数的 `AudioTrack::set` 完整签名及转发，允许 Android 16、17 的 arm64 播放器安装 PCM Hook。
- 恢复无线耳机 17.5.1 的佩戴状态主动查询，并补充新混淆布局下的游戏模式联动定位。
- 识别这份系统库已恢复的 LHDC V5 原生等价判断，将快切状态报告为 `not_required`；码率 branch 补丁仍按原路径独立处理。
- 已通过本地针对性回归、原生指令模拟和 release 构建；Android 17 的实际声音、切歌、LE Audio 与游戏切换仍待真机验证。功能梳理、证据与复测步骤见 [Android 17 适配说明](docs/android17-adaptation.md)。

## 单耳自动合并声道概览

- 在官方主面板和 OneSpace 面板新增单耳合并开关，默认关闭、按耳机保存，开启和关闭均需确认。
- 读取左右耳佩戴状态，在已适配播放器提交音频时合并左右声道；双耳佩戴时恢复立体声。
- 当前适配 QQ 音乐、网易云音乐，功能要求 Android 16 或 17 / arm64，并额外启用所用播放器的 LSPosed 作用域。
...
- 基础功能要求 Android 12 及以上；单耳自动合并声道要求 Android 16 或 17 / arm64，且播放器运行于 64 位进程。
...
- 当前只适配 QQ 音乐和网易云音乐的媒体音轨，要求 Android 16 或 17 / arm64，并能匹配系统的完整原生音频接口。未知签名仍会拒绝安装。模块其他功能的 Android 12+ 要求不代表单耳合并也支持这些旧系统。
...
| 17.0.0.103 | PMA120（本次反馈库） | 反馈确认已命中 | 原生支持，无需补丁；静态及指令模拟验证 |
...
PMA120 17.0.0.103 的快切状态通过实际代码片段及 SHA-256 指纹识别，不按 Android 17 版本号整体放行。未知 OTA 继续使用原有签名与语义扫描，未命中仍报告未适配。
